...
import { DividableByNumberRule, ContainsNumberRule, EchoRule, Rule } from '../src/rules';
import { getExpectedValueForStage2 } from './helper';
...
const containsThreeRule = new ContainsNumberRule(
  RuleType.AND,
  3,
  null,
  []
...
const fizzRule = new Rule(
  ...
  null,
  'Fizz',
  [dividableByThreeRule, containsThreeRule]
);
...
const containsFiveRule = new ContainsNumberRule(
  RuleType.AND,
  5,
  null,
  []
...
const buzzRule = new Rule(
  ...
  null,
  'Buzz',
  [dividableByFiveRule, containsFiveRule]
);
...
describe('Test fizz buzz stage 2.', () => {
  ...
  test('Should output \'Fizz\' when given 13.', () => {
  ...
  test('Should output \'FizzBuzz\' when given 51.', () => {
    const output = parse(myRule, 51);
    expect(output).toEqual('FizzBuzz');
  });

  test('Should output \'FizzBuzz\' when given 52.', () => {
    const output = parse(myRule, 52);
    expect(output).toEqual('Buzz');
  });

  test('Should output \'FizzBuzz\' when given 53.', () => {
    const output = parse(myRule, 53);
    expect(output).toEqual('FizzBuzz');
  });
  ...
  test('Should verify output when given 1 - 100.', () => {
    _.range(1, 101).forEach(input => {
      ...
      const expectedOutput = getExpectedValueForStage2(input);
